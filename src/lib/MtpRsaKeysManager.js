const { TLSerialization } = require('./tl_utils')
const { sha1BytesSync, bytesFromHex, bytesToHex, bigStringInt }  = require('./bin_utils')
  ...
  function selectRsaKeyByFingerPrint (fingerprints) {
    ...
    for (i = 0; i < fingerprints.length; i++) {
      ...
      if (foundKey = publicKeysParsed[fingerprintHex]) {
        ...
        return _.extend({fingerprint: fingerprints[i]}, foundKey)
      }
    }
