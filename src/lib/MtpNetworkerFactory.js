...
const { TLSerialization, TLDeserialization } = require('./tl_utils')
const Config = require('./Config')
const Storage = require('./Storage')
const MtpSecureRandom = require('./MtpSecureRandom')
const MtpTimeManager = require('./MtpTimeManager')
const MtpDcConfigurator = require('./MtpDcConfigurator')
const CryptoWorker = require('./CryptoWorker')
...
// .factory('MtpNetworkerFactory', function (MtpDcConfigurator, MtpTimeManager, MtpSecureRandom, Storage, CryptoWorker, AppRuntimeManager, $http, $q, $timeout, $interval, $rootScope) {
  ...
  MtpNetworker.prototype.updateSentMessage = function (sentMessageID) {
    ...
    if (sentMessage.container) {
      ...
      _.forEach(sentMessage.inner, function (innerSentMessageID) {
  ...
  MtpNetworker.prototype.pushMessage = function (message, options) {
    ...
    this.sentMessages[message.msg_id] = _.extend(message, options || {}, {deferred: deferred})
  ...
  MtpNetworker.prototype.performSheduledRequest = function () {
    ...
    _.forEach(this.pendingMessages, function (value, messageID) {
    ...
    if (messages.length > 1) {
      ...
      message = _.extend({body: container.getBytes(true)}, containerSentMessage)
    ...
    this.sendEncryptedRequest(message).then(function (result) {
      ...
      console.log(dT(), 'Encrypted request', message.msg_id);
      ...
      return requestPromise.then(
        ...
        function (error) {
          if (!error.message && !error.type) {
            ...
            error = _.extend(baseError, {type: 'NETWORK_BAD_REQUEST', originalError: error})
          }
  ...
  MtpNetworker.prototype.cleanupSent = function () {
    ...
    _.forEach(this.sentMessages, function (message, msgID) {
  ...
  MtpNetworker.prototype.processMessage = function (message, messageID, sessionID) {
    ...
    switch (message._) {
      ...
      case 'msg_new_detailed_info':
        if (this.pendingAcks.indexOf(message.answer_msg_id) >= 0) {
          break
        }
