...
const { $q, $http, $timeout } = require('./angular')
const {
  BigInteger,
} = require('../vendor/jsbn/jsbn_combined')
const {
  bytesToHex,
  bytesFromHex,
  bytesCmp,
...
//   var xhrSendBuffer = !('ArrayBufferView' in window) && (chromeVersion > 0 && chromeVersion < 30)
  ...
  function mtpSendPlainRequest (dcID, requestBuffer) {
    ...
    try {
      ...
      requestPromise = $q.reject(_.extend(baseError, {originalError: e}))
    }
    return requestPromise.then(
      function (result) {
        ...
        try {
          ...
          return $q.reject(_.extend(baseError, {originalError: e}))
        }
      ...
      function (error) {
        if (!error.message && !error.type) {
          ...
          error = _.extend(baseError, {originalError: error})
        }
